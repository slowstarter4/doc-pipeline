...
_SCHEMA_HINT = (
    ...
    "흔한데, 실제로는 INTEGER PRIMARY KEY AUTOINCREMENT를 참조하므로 number가 맞다. "
    "요청 body로 받은 memberId/bookId도 String()으로 저장·조회하지 말고 Number(...)로 "
    "변환해 sqlite 바인딩과 응답에 number로 일관되게 쓴다. node:sqlite의 lastInsertRowid는 "
    "기본적으로(setReadBigInts를 쓰지 않는 한) 이미 JS number이므로 BigInt 직렬화를 "
    "피하려고 String으로 바꿀 필요가 없다 - 바꾸면 프론트가 숫자 비교·연산을 할 때 "
    "깨진다.\n"

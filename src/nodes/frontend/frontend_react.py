...
_SCHEMA_HINT = (
    ...
    "다시 호출해서 화면을 갱신한다.\n"
    "- CSS는 src/index.css 하나에 최소한으로 넣는다. 외부 이미지는 불러오지 않는다. "
    "폰트는 디자인 토큰이 지정한 것을 쓰되, 토큰이 웹폰트(예: Google Fonts)를 지정하면 "
    "그 폰트만 <link>(index.html) 또는 @import(css)로 로드해도 된다(지정 없으면 system-ui).\n"

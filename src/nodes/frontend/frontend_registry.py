...
from .frontend_react_ts import frontend_react_ts_node

# key = .env의 FRONTEND_TARGET 값 (소문자)
FRONTEND_NODES = {
    "vanilla": frontend_node,             # 빌드 없는 단일 index.html
    "react": frontend_react_node,         # React + Vite (JSX)
    "react-ts": frontend_react_ts_node,   # React + Vite + TypeScript (TSX)
}
...
FRONTEND_RUN_INSTRUCTIONS = {
    ...
    "react-ts": "npm install && npm run dev  (기본 http://localhost:5173)",
}

...
- 새 스테이지는 전부 `src/nodes/` 밑에 노드로 추가한다. **노드는 단계별 하위 패키지로
  나눠 둔다: `docs/`(문서 생성·검토) / `backend/`(백엔드·DB·검증) / `frontend/`(프론트·검증).**
  graph.py·main.py는 `src/nodes/__init__.py`의 재수출만 쓰므로, 파일을 옮기거나 추가할 땐
  `__init__.py`에 한 줄 등록하면 되고 배선은 안 바뀐다(2026-07-24 정리). 하위 패키지 안의
  파일은 부모 import가 `...llm`/`...state`/`...design_system`(점 3개), 다른 하위 패키지 참조는
  `..backend.xxx`처럼 한다. **단 그래프 조립은
...
## 디자인 시스템 연결 (2026-07-22 구현 / 2026-07-24 확장·연결)

**2026-07-24 업데이트 — 토큰만 → 컴포넌트 레시피 + Claude Design 연결.** 토큰만 있던
design_system.md로는 프론트가 "성의없이 밋밋하게" 나왔다(CSS는 토큰을 정확히 따랐는데
디자인 자체가 안전·심심). 근본 해결로 옵션2(컴포넌트 레시피)를 택했다:
- **design_system.md를 컴포넌트 레시피로 확장.** 중립 램프(surface/border/text 계층) +
  accent/semantic 색을 `_bg` 쌍으로, 타입 스케일, 4px 간격 리듬, 2단계 그림자·포커스 링·
  120ms 전환, 그리고 **버튼(4변형+상태)·입력/폼·카드·목록/테이블·상태 배지(pill)·페이지
  헤더·빈상태/로딩/알림 배너**의 정밀 레시피. **레이아웃은 여전히 화면설계서**(복사 방지
  원칙 유지) - 배치는 화면설계서, 각 조각 생김새는 레시피.
- **Claude Design 연결됨.** `DesignSync` 도구로 "doc-pipeline design system" 프로젝트
  (projectId: e348c163-...)를 만들고 컴포넌트 프리뷰 라이브러리(`design/reference/components/`
  의 HTML 6개 + _tokens.css)를 push했다. claude.ai/design에서 시각 편집 가능한 미러다.
  방향: 지금은 로컬→Design(만들어 push), 나중에 Design에서 고치면 값을 design_system.md에
  옮겨적음(사람이 가끔). 파이프라인은 여전히 design_system.md만 읽어 자기 완결적.
- 실측: react-ts 재생성 후 디자인 토큰 17색 전부 사용, 생성 CSS에 배지·2단계 그림자·hover·
